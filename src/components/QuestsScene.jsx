...
// 1. Import ตัว Component จริงมาใช้งาน (ตรวจสอบ Path ให้ถูกต้องตามโปรเจกต์คุณนะครับ)
...
const QuestsScene = () => {
  ...
  return (
    <div className="min-h-screen  p-4 md:p-8 lg:p-12">
      ...
      {/* ใช้งาน GameWindow เป็น Modal รายละเอียด */}
      {selectedQuest && (
        <GameWindow
          isModal={true}
          title={`QUEST_LOG: ${selectedQuest.name}`}
        ...
        >
          <div className="flex flex-col gap-8">
            ...
            <div className="space-y-4">
              ...
              <p className="text-gray-700 font-thai text-lg leading-relaxed">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedQuest.tech.map((t) => (
                  <span
                    key={t}
                    className="bg-[#a29bfe15] border-[3px] border-[#a29bfe] px-3 py-1 text-xs font-bold uppercase text-[#a29bfe]"
                  >
                    {t}
                  </span>
                ))}
            ...
            <div className="mt-4 pt-8 border-t-4 border-dashed border-gray-200 flex justify-center sm:justify-end">
              <button
                ...
                className="w-full sm:w-auto bg-[#2f3542] text-white font-bold text-[12px] px-8 py-4 shadow-[5px_5px_0px_#ff7eb6] hover:translate-y-[-2px] active:translate-x-[5px] transition-all uppercase tracking-widest"
              >
                Close Log
              </button>
            </div>
...
// Sub-component QuestCard
const QuestCard = ({ name, desc, onDetails }) => (
  <div
    onClick={onDetails}
    className="bg-white border-[4px] border-[#2f3542] p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:bg-gray-50 transition-all cursor-pointer group shadow-[6px_6px_0px_rgba(0,0,0,0.05)]"
  >
    <div className="flex-1 w-full">
      <div className="flex items-center gap-4 mb-3">
        <div className="w-10 h-10 bg-green-100 border-[3px] border-green-500 flex items-center justify-center text-green-600 shrink-0 font-bold text-xl">
          ✓
        </div>
      ...
      <p className="text-lg text-gray-500 pl-14 font-thai leading-tight">
        {desc}
      </p>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mt-6 pl-0 sm:pl-14">
        <button
          ...
          className="flex items-center justify-center gap-2 font-thai text-[15px] font-bold bg-[#f1f2f6] border-[3px] border-[#2f3542] px-4 py-2.5 shadow-[4px_4px_0px_#2f3542] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all"
        >
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 font-thai text-[15px] font-bold bg-[#a29bfe20] border-[3px] border-[#2f3542] px-4 py-2.5 shadow-[4px_4px_0px_#2f3542] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all"
        >
